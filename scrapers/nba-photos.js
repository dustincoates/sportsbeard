...
function assembleUrl(playerCode){
  return 'http://i.cdn.turner.com/nba/nba/.element/img/2.0/sect/statscube/players/large/' + playerCode + '.png'
}

function fileName(playerCode){
  return playerCode + '.png'
}

function filePath(playerCode){
  return 'photos/nba/' + fileName(playerCode);
}

function savePlayersWithFileNames(players){
  fs.writeFile('data/nba.json', JSON.stringify(players), function(err){
    if(err){
      return console.log(err);
    }

    return console.log('Finished saving images for NBA.');
  })
}
...
fs.readFile('data/nba.json', function(err, data){
  if(err){
    return console.log(err);
  }
